"""
This Python program takes an image (containing text lines) that is turned. It then calculates the pitch of the text and 
turns it upright. This program does not differentiate between text that is turned upside down and text that is upright. 
It will only turn images in a way that the new image has text that is completely horizontal.
...
def dilate_image(threshedImage):
    ...
    return cv2.dilate(threshedImage, get_kernel(1, 35))
...
def do_PCA(contour):
    ...
    PCA is perfect for reducing dimensions. Our sets of pixel are scattered over a coordinate system
...
def align_image(image):
    # turn the image
    turnedImage = turn_image(image)

    # turn, thresh, dilate and erode the original image using the preprocessing methods
    preprocessedImage = erode_image(dilate_image(thresh_image(turnedImage)))
    ...
    return alignedImage, widestContour, angleInDegrees
